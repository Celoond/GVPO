...
def gvpo_compute_policy_loss(
    ...
    final_advantages = advantages.clone()

    mask_zero = (advantages == 0)
    final_advantages = torch.where(mask_zero, advantage_weight, final_advantages)

    mask_pos = (advantages > 0)
    final_advantages = torch.where(
        (mask_pos & (advantage_weight == -0.2)),
        final_advantages * 0,
        final_advantages
    )
    ...
    final_advantages = torch.where(
        (mask_neg & (advantage_weight == -0.2)),
        final_advantages * 1.2,
        final_advantages
    )
    
    negative_approx_kl = log_prob - old_log_prob
    negative_approx_kl = torch.clamp(negative_approx_kl, min=-20, max=20.0)
    ratio = torch.exp(negative_approx_kl)
    ppo_kl = verl_F.masked_mean(-negative_approx_kl, response_mask)

    pg_losses1 = -final_advantages * ratio
    if cliprange_low is None:
        cliprange_low = cliprange
    if cliprange_high is None:
        cliprange_high = cliprange
    pg_losses2 = -final_advantages * torch.clamp(ratio, 1 - cliprange_low, 1 + cliprange_high)
...
def compute_policy_loss(
    ...
    """
    assert clip_ratio_c > 1.0, "The lower bound of the clip_ratio_c for dual-clip PPO should be greater than 1.0," + f" but get the value: {clip_ratio_c}."
    adv_mask = (advantages.abs() >= 0.01).float()
    effective_mask = response_mask * adv_mask
    negative_approx_kl = log_prob - old_log_prob
    ratio = torch.exp(negative_approx_kl)
...
def compute_bandit_policy_loss(
    old_log_prob,
    log_prob,
    advantages,
    response_mask,
    ...
    """
    Bandit-style PPO loss using sequence-level probability ratios.
    Args:
        old_log_prob: shape (bs, seqlen)
        log_prob: shape (bs, seqlen)
        advantages: shape (bs,) or (bs, 1)
        response_mask: shape (bs, seqlen), 1 for valid tokens
        cliprange*, clip_ratio_c: same as before
    ...
    log_prob_seq = (log_prob * response_mask).sum(dim=1)
    old_log_prob_seq = (old_log_prob * response_mask).sum(dim=1)

    negative_approx_kl = log_prob - old_log_prob
    ppo_kl = verl_F.masked_mean(-negative_approx_kl, response_mask)

    ratio = torch.exp(log_prob_seq - old_log_prob_seq)

    advantages = (advantages * response_mask).sum(dim=1) / response_mask.sum(dim=1).clamp(min=1)
    ...
    pg_clipfrac = (pg_loss2 > pg_loss1).float().mean()

    pg_loss3 = -advantages * clip_ratio_c
    clip_pg_loss2 = torch.minimum(pg_loss3, clip_pg_loss1)
    pg_clipfrac_lower = ((clip_pg_loss1 > pg_loss3) & (advantages < 0)).float().mean()

    pg_loss = torch.where(advantages < 0, clip_pg_loss2, clip_pg_loss1).mean()
...
def compute_value_loss(vpreds: torch.Tensor, returns: torch.Tensor, values: torch.Tensor, response_mask: torch.Tensor, cliprange_value: float, loss_agg_mode: str = "token-mean"):
    ...
    Args:
        ...
        response_mask: `(torch.Tensor)`
            Mask for tokens to calculate value function losses.
        loss_agg_mode: (str) see `agg_loss`
